...
def collect_inven_news_links_from_list():
...
def crawl_inven_news_by_date(target_date):
    results = []
    article_items = collect_inven_news_links_from_list()
...
def run_summary(target_date, label):
    with st.spinner(f"正在收集并分析{label}的主要新闻，请稍候..."):
        results = crawl_inven_news_by_date(target_date)
