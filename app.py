...
"""
    ...
    return fallback_korean_summary(content)
...
def crawl_inven_hot_news_by_date(target_date):
    ...
    for item in article_items:
        try:
            ...
            korean_summary = summarize_article(real_title, content)
...
def run_summary(target_date, label):
    with st.spinner(f"正在收集并分析{label}的主要新闻，请稍候..."):
        ...
        if df.empty:
            ...
            excel_file = f"inven_hot_news_summary_ko_{target_date}.xlsx"
...
st.write("本系统整理韩国 Inven 游戏新闻中“主要新闻”内容，并生成韩文一行摘要。")
